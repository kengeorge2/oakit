              ...
              <Reveal key={post.slug} delay={idx as 0 | 1 | 2}>
                <div className="card-glass rounded-xl overflow-hidden h-full flex flex-col">
                  {post.feature_image ? (
                    ...
                    <div className="rounded-t-lg bg-gray-800/50 aspect-video flex items-center justify-center">
                      <span className="text-gray-500 text-sm">No image</span>
                    </div>
                  )}
                  <div className="space-y-3 p-6 flex flex-col flex-1">
                    {post.tags && post.tags.length > 0 && (
                      <span className="inline-block w-fit rounded-full bg-purple-500/10 border border-purple-500/20 px-3 py-1 text-xs font-medium text-purple-300">
                        {post.tags[0].name}
                      </span>
                    )}
        ...
        {posts.length > 0 && (
          <Reveal>
            <div className="flex justify-center mt-10">
              <Link
                href="/blog"
                className="inline-flex items-center justify-center rounded-lg border border-white/[0.1] bg-white/[0.04] px-6 py-3 text-sm font-medium text-gray-300 hover:bg-white/[0.08] hover:text-white hover:border-white/[0.2] transition-all duration-300"
              >
                View All Posts
              </Link>
            </div>
